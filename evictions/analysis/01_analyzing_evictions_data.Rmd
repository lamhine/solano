...
acs_wide_cty <- acs_wide_cty[c(1:26,51:53,56:70)]
...
df %<>%
  mutate(filing_rate = (filings_observed/renter_hhs)*100)
...
# reducing cols for simplicity
acs_wide_trct <- acs_wide_trct[c(1:26,51:53,56:70)]
...
df_tract %>%
  subset(county=="Solano County" & year==2014) %>%
  arrange(desc(pnhw)) %>%
  select(tract, county, year, filings, filing_rate, filing_rate_2, p_cb_renter, prent_blk, prent_his, pnhb, phis, pnhw, prent)

df %>%
  subset(county=="Solano County") %>%
  arrange(desc(filings_observed)) %>%
  select(county, year, filings_observed, filing_rate, p_cb_renter, prent_blk, prent_his, pnhb, phis, pnhw, prent)

df_tract %>%
  subset(county=="Solano County" & is.na(filings)) # 48 tract-years
```

Saving out final county-level and tract-level data files.

```{r}
# data was showing up weird in Tableau so getting rid of NANs
df_tract %<>% mutate_all(~ifelse(is.nan(.), NA, .))

load("data/df_county.RData")
...
# loading data and recomputing filing rate like I do for the county data
load("data/df_tract.RData")
df_tract %<>%
  mutate(filing_rate_2 = (filings/renter_hhs)*100)

df_tract %>%
  subset(county=="Solano County") %>%
  ggplot(aes(x = pnhb, y = filing_rate)) + 
  geom_point() +
  theme_classic() +
  xlab("Percent Black") + ylab("Filing Rate per 100 Renting Households")

df_tract %>%
  subset(county=="Solano County") %>%
  ggplot(aes(x = phis, y = filing_rate)) + 
  geom_point() +
  theme_classic() +
  xlab("Percent Latino") + ylab("Filing Rate per 100 Renting Households")

df_tract %>%
  subset(county=="Solano County") %>%
  ggplot(aes(x = pnhw, y = filing_rate)) + 
  geom_point() +
  theme_classic() +
  xlab("Percent White") + ylab("Filing Rate per 100 Renting Households")
  

df_tract %>%
  select(tract,year,filings,renter_hhs,filing_rate,filing_rate_2)

tract <- df_tract %>%
  subset(county=="Solano County")
cor.test(tract$filing_rate,tract$pnhb, use = "pairwise.complete.obs")
cor.test(tract$filing_rate,tract$phis, use = "pairwise.complete.obs")
cor.test(tract$filing_rate,tract$pnhw, use = "pairwise.complete.obs")
...
tract %>%
  arrange(desc(med_hh_inc_rent)) %>%
  select(tract,year,filings,filing_rate,med_hh_inc_rent,pnhb,pnhw,phis)
```
